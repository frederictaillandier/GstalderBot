...
def test_trim_date_doesnt_corrupt_data():
    ...
    mock = {"date": "2021-01-01", "name": "Gilles", "waste_type": 0}
    trimmed_data = trim_date(mock)
    assert (
        trimmed_data["date"] == datetime.datetime(2021, 1, 1)
        and trimmed_data["waste_type"] == 0
    )
...
def test_trim_schedule_removes_unnecessary_data():
    ...
    mock = [
        {"date": "2021-01-01", "name": "Gilles", "waste_type": 1},
        {"date": "2021-01-02", "name": "Tony", "waste_type": 0},
    ]
...
def test_adliswil_trash_schedule_grabber_returns_waste_type_1_in_june():
    ...
    schedule = grabber.grab(
        datetime.datetime(2023, 6, 1), datetime.datetime(2023, 6, 2)
    )
